...
;; variables defining the citizen
...
to go
  ...
  ;; to stop
  if inf = population-size or inf = 0 [stop]
  ...
  [
    ...
    ask citizens with [epidemic-state = "Recovering"]
  [
    recover

  ]
    ask citizens with [epidemic-state = "Recovered"]
  [
    rt random 100 lt random 100 fd 1
    avoid-walls
  ]
...
;; virus transmission
to get-virus
  let target one-of other citizens in-radius transmission-distance with [epidemic-state = "Infected"]
  if is-agent? target
  [
    if ([epidemic-state] of target = "Infected" and random-float 100 < probability-transmission)
    or
    ([epidemic-state] of target = "Carrier" and random-float 1 < proba-transmission-unreported-infected) [
...
to setup-globals
  ...
  ;set population-size 153
  ;set nb-infected-initialisation 10
  ;set distanciation-distance 1.5
  ; set probability-transmission
...
;; defining the population specifics
...
to show-epidemic-state
  (ifelse
    ...
    epidemic-state = "Recovering" [ set color lput transparency extract-rgb blue ]

    epidemic-state = "Recovered" [ set color lput transparency extract-rgb white ]

    epidemic-state = "Carrier" [set color lput transparency extract-rgb yellow ]
...
;; quarantine infected agents
to quarantine-infected

 if isolate? = true [
    if (infection-date + discovery-period) <= ticks and not quarantined?   [



    set epidemic-state "Recovering"
...
to recover
    if (recovery-duration + discovery-period + infection-date) = ticks and quarantined?
   [
  ask citizens with [epidemic-state = "Recovering"]
    [
...
to interventions-off
  ...
  set isolate? false
  set recovery-duration 0
  set infected-avoidance-distance 0
  set discovery-period 7
  set transmission-distance 1
  set %respect-distanciation 0
...
to-report inf
  ...
  report count citizens with [epidemic-state = "Infected"]
...
to-report iso
  ...
  report count citizens with [epidemic-state = "Recovering"]
...
"" ""
PENS
"Inf" 1.0 0 -2674135 true "" "if inf > 0 [plot inf]"
"S" 1.0 0 -10899396 true "" "if sus > 0 [plot sus]"
...
"" ""
PENS
"R" 1.0 0 -16777216 true "" "if rec > 0 [plot inf]"
"Iso" 1.0 0 -7500403 true "" "if iso > 0 [plot inf]"
